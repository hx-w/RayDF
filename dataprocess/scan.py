...
def get_image_plane_positions(pos, dir, near, fov, resol_x, resol_y):
    # 计算相机的右方向
    up = np.array([0, 0, 1])
    camera_forward = -dir / np.linalg.norm(dir)
    right = np.cross(camera_forward, up)

    # 计算成像平面上像素的尺寸
    pixel_width = 2 * near * np.tan(fov / 2) / (resol_x - 1)
    pixel_height = 2 * near * np.tan(fov / 2) / (resol_y - 1)
    
    half_width = pixel_width * (resol_x - 1) / 2
    half_height = pixel_height * (resol_y - 1) / 2

    # 计算成像平面上像素中心的坐标
    image_plane_positions = []
    for i in range(resol_x):
        for j in range(resol_y):
            center_x = (resol_x - 1 - i) * pixel_width - half_width
            center_y = (resol_y - 1 - j) * pixel_height - half_height
            image_plane_position = pos + near * dir + center_y * right + center_x * up
            image_plane_positions.append(image_plane_position)

    return np.array(image_plane_positions)
